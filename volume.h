...
public:
    ...
    float getSpacingX() { return spcx; }
    float getSpacingY() { return spcy; }
    float getSpacingZ() { return spcz; }
        ...
        for (int i = 1; i < sizex - 1; i++)
            for (int j = 1; j < sizey - 1; j++)
                for (int k = 1; k < sizez - 1; k++) {
                    ...
                    float v = getDataAt(i - 1, j, k) + getDataAt(i + 1, j, k) +
                              getDataAt(i, j - 1, k) + getDataAt(i, j + 1, k) +
                              getDataAt(i, j, k - 1) + getDataAt(i, j, k + 1);
    ...
    double getDataAt(int index) { return data[index]; }
    ...
    /* Write to file */
    void toMathematicaFile(char* fname, int lx, int hx, int ly, int hy, int lz,
                           int hz) {
        FILE* fout = fopen(fname, "w");
    ...
    void toMRCFile(char* fname) {
        ...
        float ds[3] = {(float)dmin, (float)dmax, 0.0f};
